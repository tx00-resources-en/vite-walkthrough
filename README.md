...
Open a browser to [http://localhost:5173]. You should see `Vite + React`
underneath the two logos. Congratulations! You've just created your first React
app with Vite!
...
You can also have Vite open your app in a browser window when it starts the
server, a nice little convenience.
...
__index.html__ since that is your app's entry file. The only thing you need to
change here is the `title`: make it "App Academy Template" or "My Template"
instead of "Vite + React". While you're in this file, look at the `script` in
the `body`. It loads __src/main.jsx__ and designates it as type `module`. This
is how your app gets loaded into the browser.
...
```sh
npx degit <YOUR-GH-USERNAME>/my-react-vite-template#main <new-project-name>
```
...
- [How to Setup Eslint in a Vite React Project](https://ghostxd.hashnode.dev/how-to-setup-eslint-in-a-vite-react-project)
- [App academy: vite walkthrough](https://github.com/appacademy/aa34-vite-walkthrough/tree/campus-hybrid)
